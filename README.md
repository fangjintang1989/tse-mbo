...
| `output/audit/step2_order_book_audit.csv` | Per-price-level audit for every replayed issue (`cum_bid`, `cum_ask`, `tip_up`, `tip_down`, `selected=yes` on the chosen IAP row) |
| `output/audit/step1_fixture_report.txt` | Capture-input summary: record counts, FLEX tag-type counts, per-capture issue-code → venue-name mappings |
...
**Supplementary audit artifacts.** The fixture test writes four files under `build/results/` on every run. Two of the smaller ones are also committed under `output/audit/` so a reviewer can inspect them without running the tests:

- `step1_decoded_messages.csv` — one row per decoded FLEX tag (~107 MB; **`build/results/` only**, not committed).
- `step1_fixture_report.txt` — counts, endpoints, per-capture issue-code → venue-name mappings. Committed as `output/audit/step1_fixture_report.txt`.
- `step2_order_book_audit.csv` — final per-price-level audit with `cum_bid`, `cum_ask`, `tip_up`, `tip_down`; the `selected=yes` row explains the chosen IAP/IAV for each symbol. Committed as `output/audit/step2_order_book_audit.csv`.
- `step3_iap_iav_fixture_results.csv` — same per-stock output as the deliverable, regenerated by the test (`build/results/` only; the committed version lives at `output/iap_iav_20241105.csv`).
...
output/
  ...
  audit/
    step2_order_book_audit.csv   per-price-level ladder for every replayed issue (4.8 MB)
    step1_fixture_report.txt     capture-input summary: record counts, endpoints, issue maps
data/
